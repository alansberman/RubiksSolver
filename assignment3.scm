...
;---------------------------QUESTION 3.1-----------------------
;Helper function for solveCube
;Checks if the first state in a list (x) is in the list of solved states (y)
;if x is found in y, x's corresponding move history is returned
;else, recurse with the remaining states and their remaining histories
;if x is not found in y, ("?") is returned, indicating more move(s) are needed
(define (solveCubeHelper solved aList)
  (if 
	(or (null? aList) (null? (car aList)))
 		'(("?"))
    		(if (in (car(car aList)) solved )
        		(car (cadr aList))
            		(solveCubeHelper solved (list (cdr (car aList)) 
                        (cdr (cadr aList))))   
   		)
  )
)
;Solves a rubiks cube using breadth first search. Can solve up to roughly 7 moves.
;Uses the return value of solveCubeHelper
;If the above value is a ("?"), additional move(s) are needed
;Else the solution has been found (a move history) and is returned
(define (solveCube solved initial n)
	(let ([ans (solveCubeHelper solved (genStates n initial '()))])
   	 	(if
			 (and (equal? ans '(("?"))) (< n 8))
